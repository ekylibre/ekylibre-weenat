...
module Weenat
  ...
  class WeenatIntegration < ActionIntegration::Base
    ...
    BASE_URL = 'https://api-prod.weenat.com/api'.freeze
    ...
    def fetch_all
      ...
      # for testing
      # call = RestClient::Request.execute(method: :get, url: PLOTS_URL, headers: {Authorization: "JWT #{t}"})
    ...
    # Get last_values historical values of one plot for 35 days max
    ...
    def last_values(plot_id, started_at, stopped_at)
      ...
      # Call API
      get_json("#{PLOTS_URL}#{plot_id}/measures/?start=#{started_at}&end=#{stopped_at}", 'Authorization' => "JWT #{token}") do |r|
        r.success do
          list = JSON(r.body).deep_symbolize_keys
        end
